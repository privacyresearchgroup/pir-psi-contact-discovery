...
//		- cfilter.MetadataSize(uint8) sets the size of the metadata
...
func (cf *CFilter) Insert(item []byte) bool {
	f := fprint(item, cf.fpSize, cf.metaSize, cf.hashfn)
	j := hashfp(item) % cf.size
	k := (j ^ hashfp(getKey(f))) % cf.size
...
func (cf *CFilter) Buckets() []bucket {
	return cf.buckets
}

func (cf *CFilter) ID(item []byte) (uint, uint) {
	f := fprint(item, cf.fpSize, cf.metaSize, cf.hashfn)
	j := hashfp(item) % cf.size
	k := (j ^ hashfp(getKey(f))) % cf.size
...
func (cf *CFilter) FP(item []byte) fingerprint {
	f := fprint(item, cf.fpSize, cf.metaSize, cf.hashfn)
	return f
}


func (cf *CFilter) FPKey(f fingerprint) []byte {
	return getKey(f)
}

func (cf *CFilter) Metadata(f fingerprint) []byte {
	return getMeta(f)
}
