...
func NewPuncHintReq(randSource *rand.Rand) *PuncHintReq {
	...
	fmt.Print("Total bits for client to request hint: ")
	fmt.Println(len(req.RandSeed)*8	 + 64)
	return req
}
...
func (req *PuncHintReq) Process(db StaticDB) (HintResp, error) {
	...
	totalhints := (len(hints)*len(hints[0])*8)
	fmt.Print("Total bits for server to send hints: ")
	fmt.Println(totalhints + 64 * 3 + len(req.RandSeed)*8)
...
func (c *puncClient) Query(i int) ([]QueryReq, ReconstructFunc) {
	...
	fmt.Print("Total bits for online client query: ")
	fmt.Println((64*3 + 32)*2 + 64*2 + len(puncSetL.Keys)*8 + len(puncSetR.Keys)*8)
...
func (q *PuncQueryReq) Process(db StaticDB) (interface{}, error) {
	...
	fmt.Print("Total number of bits for server to answer query: ")
	fmt.Println(len(resp.Answer) * 8 + len(resp.ExtraElem)*8)
